...
Vector meanVarWeights(Vector const& assetRets, Vector const& assetVols, Matrix const& correlMat, double lambda)
{
  ...
  Matrix SigmaInv = Sigma.i();

  Vector sigmaInvMu = SigmaInv * assetRets;
  Vector sigmaInvIota = SigmaInv * iota;

  double c = dot(iota, sigmaInvIota);
  double a = dot(iota, sigmaInvMu);

  Vector w_mvp = (1.0 / c) * sigmaInvIota;
  Vector w = w_mvp + lambda * (sigmaInvMu - a * w_mvp);

  return w;
}

std::tuple<Vector, Vector, Vector> 
meanVarFront(Vector const& assetRets, Vector const& assetVols, Matrix const& correlMat, double lambdaMax, int nLambdaSteps)
{
  ORF_ASSERT(lambdaMax >= 0, "Lambda max must be non-negative");
  ORF_ASSERT(nLambdaSteps > 0, "Number of steps must be positive");

  size_t N = (size_t)nLambdaSteps + 1;
  Vector meanVec(N);
  Vector stdDevVec(N);
  Vector lambdaVec(N);
  ...
  for (size_t i = 0; i < N; ++i) {
    double lam = i * dLambda;
    
    Vector w = meanVarWeights(assetRets, assetVols, correlMat, lam);


    std::tuple<double, double> riskStats = ptRisk(w, assetRets, assetVols, correlMat);

    meanVec[i]   = std::get<0>(riskStats);
    stdDevVec[i] = std::get<1>(riskStats);
    lambdaVec[i] = lam;
  }

  return std::make_tuple(meanVec, stdDevVec, lambdaVec);
}
