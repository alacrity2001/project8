...
def meanVarWghts(assetrets, assetvols, correlmat, risk_aversion):
    """Weights of the mean-variance efficient portfolio for a specific risk aversion level.
    ...
    """
    return pyorflib.meanVarWghts(assetrets, assetvols, correlmat, risk_aversion)


def meanVarFront(assetrets, assetvols, correlmat, lambdamax, nsteps):
    """Computes the mean-variance efficient frontier.
    ...
    correlmat : 2D numpy array
        asset return correlation matrix
    lambdamax : double
        maximum risk aversion coefficient to compute
    nsteps : int
        number of steps for lambda discretization

    Returns
    -------
    dictionary
        Mean : 1D array of mean portfolio returns
        StdDev : 1D array of portfolio return standard deviations
        Lambda : 1D array of lambda values used
    """
    return pyorflib.meanVarFront(assetrets, assetvols, correlmat, lambdamax, nsteps)



def cbBSPDE(facevalue, maturity, convratio, convstart, convend, 
            callstrike, callstart, callend, 
            spot, discountcrv, divyield, volatility, 
            pdeparams, allresults=False):
    """Price of a Convertible Bond in the Black-Scholes model using finite difference PDE.
    ...
    convratio : double
        number of shares received per bond unit
    convstart : double
        start time of conversion period in years
    convend : double
        end time of conversion period in years
    callstrike : double
        value at which the issuer may redeem the bond
    callstart : double
        start time of call period in years
    callend : double
        end time of call period in years
    spot : double
        asset spot price
    discountcrv : str
        discount yield curve name
    ...
    The keys `Times`, `Spots` and `Values` are available only if `allresults`==True.
    """
    return pyorflib.cbBSPDE(facevalue, maturity, convratio, convstart, convend, 
                            callstrike, callstart, callend, 
                            spot, discountcrv, divyield, volatility, 
                            pdeparams, allresults)